...
function PanelInner({ resource, onClose }) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Body */}
      <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5 min-h-0" style={{ scrollbarWidth: 'thin' }}>
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="hidden md:flex fixed z-[1002] flex-col"
        style={{
          left: '16px',
          top: 'calc(64px + 16px)',
          width: '320px',
          ...
          boxShadow: '0 24px 60px rgba(0,0,0,0.16), 0 0 0 1px rgba(255,255,255,0.8)',
        }}
        aria-label={`Recurso: ${resource.name}`}
      >
        <div className="dark:hidden flex flex-col h-full min-h-0">
          <PanelInner resource={resource} onClose={onClose} />
        </div>
        <div className="hidden dark:flex flex-col h-full min-h-0" style={{ color: 'inherit' }}>
          <PanelInner resource={resource} onClose={onClose} />
        </div>
      </motion.div>
