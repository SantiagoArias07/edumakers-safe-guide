...
# Parse CORS origins — strip whitespace and accidental quotes from env var editors
_raw_origins = os.getenv("CORS_ORIGINS", "http://localhost:5173,http://127.0.0.1:5173")
CORS_ORIGINS = [o.strip().strip("\"'") for o in _raw_origins.split(",") if o.strip()]

print(f"[SafeGuide] CORS origins loaded: {CORS_ORIGINS}", flush=True)
...
def startup():
    create_tables()
    print(f"[SafeGuide] Startup complete. Allowing origins: {CORS_ORIGINS}", flush=True)
